...
function initDatatable(doc, count) {
    ...
    data.push(doc.day);
    data.push(doc.time);
    table.row.add(['<input type="checkbox" class="checkbox-datatable" value="' + data + '">', count, doc.topic, doc.name, '<img class="table-image" src="/assets/images/wait.png" style="width : 45px; height:45px;">', doc.date, doc.note_ID, doc.student_ID, doc.teacher, doc.date, doc.time, doc.detail, doc.status, doc.day, doc.section, doc.advisorName]).draw();
}
