...
function initDatatable(doc, count) {
    ...
    data.push(doc.teacher);
    data.push(doc.day);
    data.push(doc.time);
    doc.tool.forEach(forEachArrayTools);
    doc.toolNumber.forEach(forEachArrayNumberOfTools);
    table.row.add(['<input type="checkbox" class="checkbox-datatable" value="' + data + '">', count, toolsArray, doc.name, '<img class="table-image" src="/assets/images/wait.png" style="width : 45px; height:45px;">', doc.date, doc.note_ID, doc.student_ID, doc.teacher, doc.date, doc.time, doc.topic, doc.status, doc.day, doc.dateBack, NumberArray, doc.section, doc.advisorName]).draw();
}
...
function decline(note) {
    Swal.fire({
        ...
        input: 'select',
        inputOptions: {
            อุปกรณ์มีไม่เพียงพอ: 'อุปกรณ์มีไม่เพียงพอ',
            ไม่สะดวกในเวลานี้: 'ไม่สะดวกในเวลานี้',
            อื่นๆ: 'อื่นๆ',
        },
