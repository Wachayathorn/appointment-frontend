function getCookie(cname) {
    var name = cname + "=";
    var decodedCookie = decodeURIComponent(document.cookie);
    var ca = decodedCookie.split(';');
    for (var i = 0; i < ca.length; i++) {
        var c = ca[i];
        while (c.charAt(0) == ' ') {
            c = c.substring(1);
        }
        if (c.indexOf(name) == 0) {
            return c.substring(name.length, c.length);
        }
    }
    return "";
};

function RequestAppointmentTeacher() {
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            var user = firebase.auth().currentUser;
            if (user) {
                var count = 0;
                var uID = getCookie('uID');
                var name = getCookie('User');
                var role = getCookie('Role');
                document.getElementById("user_login").innerHTML = name;
                // User is signed in.
                if (uID != "") {
                    $.ajax({
                        url: "https://us-central1-appointment-c8cfd.cloudfunctions.net/API/requestAccept",
                        type: "POST",
                        data: {
                            'uID': uID,
                            'role': role,
                        },
                        success: function (data) {
                            if (data) {
                                if (data.allAppData) {
                                    var allAppointment = data.allAppData;
                                    for (i = 0; i < allAppointment.length; i++) {
                                        count++;
                                        initDatatable(allAppointment[i], count);
                                    }
                                }
                            } else {
                                Swal.fire({
                                    type: 'error',
                                    title: 'ERROR',
                                    text: 'Error !',
                                    confirmButtonText: '<i class="fa fa-times"></i> ตกลง',
                                }).then(function () {
                                    RequestAppointmentTeacher();
                                })
                            }
                        },
                        error: function (xhr, status, error) {
                            console.log('Error: ' + error.message);
                        },
                    });
                }
                else {
                    console.log("User is null");
                }
            }
        } else {
            // No user is signed in.
            console.log("No user signed in");
            var name = getCookie('User');
            if (!name) {
                Swal.fire({
                    type: 'error',
                    title: 'ERROR',
                    text: 'กรุณาเข้าสู่ระบบก่อนใช้งาน !',
                    confirmButtonText: '<i class="fa fa-times"></i> ตกลง',
                }).then(function () {
                    window.location.href = "/index.html";
                })
            } else {
                window.location.href = "/index.html";
            }
        }
    });
}

function initDatatable(doc, count) {
    table.row.add([count, doc.topic, doc.name, '<img class="table-image" src="/assets/images/accept.png" style="width : 45px; height:45px;">', doc.date, '<button class="btn btn-success" value="' + doc.note_ID + '"  onclick="callFile(this.value)"><img src="/assets/images/xlsx.png" height=15px" width="15px"></button>', doc.note_ID, doc.student_ID, doc.teacher, doc.date, doc.time, doc.detail, doc.status, doc.day, doc.section, doc.advisorName]).draw();
}

function rowChild(obj) {
    return '<h5 style="margin-top:10px;"><b>หัวข้อ : </b>' + obj["Topic"] + '</h5>' + '<p><b>ชื่อ : </b>' + obj["Name"] + '<b style="margin-left:10px;">รหัสนักศึกษา : </b>' + obj["Student_ID"] + '</p>' + '<p><b>กลุ่ม : </b>' + obj["Section"] + '<b style="margin-left:10px;">อาจารย์ที่ปรึกษา : </b>' + obj["Advisor"] + '</p>' + '<p><b>นัดหมายอาจารย์ชื่อ : </b>' + obj["Teacher"] + '</p>' + '<p><b>วัน : </b>' + obj["Day"] + '<b style="margin-left:10px;">วันที่ : </b>' + obj["Date"] + '</p>' + '<p><b>เวลา : </b>' + obj["Time"] + '</p>' + '<p><b>สถานะ : </b>' + obj["Status"] + '</p>' + '<p><b>รายละเอียดการนัดหมาย : </b>' + obj["Detail"] + '</p>';
}

var nameStudent;
var student_id;
var nameTeacher;
var _topic;
var _detail;

function callFile(note) {
    var user = firebase.auth().currentUser;
    if (user) {
        var role = getCookie('Role');

        $.ajax({
            url: "https://us-central1-appointment-c8cfd.cloudfunctions.net/API/excelTeacher",
            type: "POST",
            data: {
                'role': role,
                'note_id': note,
            },
            success: function (data) {
                if (data) {
                    nameStudent = data.nameStudent;
                    student_id = data.student_id;
                    nameTeacher = data.nameTeacher;
                    _topic = data._topic;
                    _detail = data._detail;

                    createExcel();
                }
            },
            error: function (xhr, status, error) {
                console.log('Error: ' + error.message);
            },
        });
    } else {
        // No user is signed in.
        console.log("No user signed in");
        var name = getCookie('User');
        if (!name) {
            Swal.fire({
                type: 'error',
                title: 'ERROR',
                text: 'กรุณาเข้าสู่ระบบก่อนใช้งาน !',
                confirmButtonText: '<i class="fa fa-times"></i> ตกลง',
            }).then(function () {
                window.location.href = "/index.html";
            })
        } else {
            location.reload();
        }
    }
}

async function createExcel() {
    const workbook = new ExcelJS.Workbook();

    // Insert image
    var myBase64Image = "data:image/jpeg;base64,";
    var img_logo = workbook.addImage({
        base64: myBase64Image,
        extension: 'png',
    });

    var worksheet = workbook.addWorksheet('เอกสารการนัดหมาย', {
        pageSetup: { paperSize: 9 },
        views: [{ showGridLines: false, style: 'pageLayout' }],
    });

    // adjust pageSetup settings afterwards
    worksheet.pageSetup.margins = {
        left: 0.9, right: 0.7,
        top: 0.75, bottom: 0.75,
        header: 0.3, footer: 0.3
    };

    // Set style
    worksheet.getCell('A1:I32').font = {
        name: 'TH Sarabun New',
        family: 4,
        size: 16,
    };
    ////////////
    // Images
    worksheet.addImage(img_logo, 'A1:A3');

    worksheet.getCell('B2').value = "คณะวิศวกรรมศาสตร์";
    worksheet.getCell('B2').font = {
        bold: true
    };

    worksheet.getCell('B3').value = "มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี";
    worksheet.getCell('B3').font = {
        bold: true
    };

    worksheet.getCell('I3').value = "วศ-อ-01";
    worksheet.getCell('I3').alignment = { horizontal: 'right' };
    worksheet.getCell('I3').font = {
        bold: true
    }

    worksheet.mergeCells('A5:I5');
    worksheet.getCell('A5').value = "แบบบันทึกการบริการให้คำปรึกษา คณะวิศวกรรมศาสตร์";
    worksheet.getCell('A5').alignment = { horizontal: 'center' };
    worksheet.getCell('A5').font = {
        bold: true
    }

    worksheet.mergeCells('A6:I6');
    worksheet.getCell('A6').value = "มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี";
    worksheet.getCell('A6').alignment = { horizontal: 'center' };
    worksheet.getCell('A6').font = {
        bold: true
    }

    worksheet.mergeCells('A7:I7');
    worksheet.getCell('A7').value = "ประจำปีการศึกษา ...............";
    worksheet.getCell('A7').alignment = { horizontal: 'center' };
    worksheet.getCell('A7').font = {
        bold: true
    }

    worksheet.mergeCells('H8:I8');
    worksheet.getCell('H8').value = "ครั้งที่........../..........";
    worksheet.getCell('H8').alignment = { horizontal: 'right' };
    worksheet.getCell('H8').font = {
        bold: true
    }

    worksheet.mergeCells('A9:I9');
    worksheet.getCell('A9').value = "…..................................................................................................................................................................................................................";
    worksheet.getCell('A9').alignment = { vertical: 'middle', horizontal: 'center' };

    worksheet.mergeCells('A10:B10');
    worksheet.getCell('A10').value = "ข้อมูลนักศึกษา";
    worksheet.getCell('A10').alignment = { horizontal: 'left' };
    worksheet.getCell('A10').font = {
        bold: true
    }

    worksheet.getCell('A11').value = "ชื่อ - สกุล";
    worksheet.getCell('A11').alignment = { horizontal: 'left' };

    worksheet.mergeCells('B11:D11');
    worksheet.getCell('B11').value = nameStudent;
    worksheet.getCell('B11').alignment = { horizontal: 'center' };

    worksheet.mergeCells('E11:F11');
    worksheet.getCell('E11').value = "รหัสนักศึกษา";
    worksheet.getCell('E11').alignment = { horizontal: 'right' };

    worksheet.mergeCells('G11:I11');
    worksheet.getCell('G11').value = student_id;
    worksheet.getCell('G11').alignment = { horizontal: 'center' };

    worksheet.mergeCells('A12:B12');
    worksheet.getCell('A12').value = "นักศึกษาภาควิชา";
    worksheet.getCell('A12').alignment = { horizontal: 'left' };

    worksheet.mergeCells('C12:D12');
    worksheet.getCell('C12').value = "วิศวกรรมคอมพิวเตอร์";
    worksheet.getCell('C12').alignment = { horizontal: 'left' };

    worksheet.getCell('E12').value = "สาขาวิชา";
    worksheet.getCell('E12').alignment = { horizontal: 'left' };

    worksheet.mergeCells('F12:H12');
    worksheet.getCell('F12').value = "-";
    worksheet.getCell('F12').alignment = { horizontal: 'left' };

    worksheet.mergeCells('A13:B13');
    worksheet.getCell('A13').value = "ชั้นปีที่….........................";
    worksheet.getCell('A13').alignment = { horizontal: 'left' };

    worksheet.getCell('C13').value = "รอบ";
    worksheet.getCell('C13').alignment = { horizontal: 'right' };

    worksheet.mergeCells('D13:H13');
    worksheet.getCell('D13').value = "( ) ปกติ     ( ) บ่าย     ( ) สมทบ";
    worksheet.getCell('D13').alignment = { horizontal: 'left' };

    worksheet.mergeCells('A14:E14');
    worksheet.getCell('A14').value = "ขอรับการให้คำปรึกษาในเรื่อง (สรุปปัญหาโดยย่อ)";
    worksheet.getCell('A14').alignment = { horizontal: 'left' };

    worksheet.mergeCells('F14:I14');
    worksheet.getCell('F14').value = "เรื่อง : " + _topic;
    worksheet.getCell('F14').alignment = { horizontal: 'left' };

    worksheet.mergeCells('A15:I17');
    worksheet.getCell('A15').value = "รายละเอียด : " + _detail;
    worksheet.getCell('A15').alignment = { vertical: 'top', horizontal: 'left', wrapText: true };

    worksheet.getCell('F18').value = "ลงชื่อ";
    worksheet.getCell('F18').alignment = { horizontal: 'right' };

    worksheet.mergeCells('G18:I18');
    worksheet.getCell('G18').value = "…..................................................................";
    worksheet.getCell('G18').alignment = { horizontal: 'center' };

    worksheet.mergeCells('G19:I19');
    worksheet.getCell('G19').value = "( " + nameStudent + " )";
    worksheet.getCell('G19').alignment = { horizontal: 'center' };

    worksheet.mergeCells('A20:I20');
    worksheet.getCell('A20').value = "บันทึกของอาจารย์ที่ปรึกษา/ผู้ให้คำปรึกษา";
    worksheet.getCell('A20').alignment = { horizontal: 'left' };
    worksheet.getCell('A20').font = {
        bold: true
    }

    worksheet.mergeCells('A21:I23');
    worksheet.getCell('A21').alignment = { vertical: 'top', horizontal: 'left' };

    worksheet.getCell('F24').value = "ลงชื่อ";
    worksheet.getCell('F24').alignment = { horizontal: 'right' };

    worksheet.mergeCells('G24:I24');
    worksheet.getCell('G24').value = "…..................................................................";
    worksheet.getCell('G24').alignment = { horizontal: 'center' };

    worksheet.mergeCells('G25:I25');
    worksheet.getCell('G25').value = "( " + nameTeacher + " )";
    worksheet.getCell('G25').alignment = { horizontal: 'center' };

    worksheet.mergeCells('G26:I26');
    worksheet.getCell('G26').value = "อาจารย์ที่ปรึกษา/ผู้ให้คำปรึกษา";
    worksheet.getCell('G26').alignment = { horizontal: 'center' };

    worksheet.getCell('A27').value = "ติดตามผล";
    worksheet.getCell('A27').alignment = { horizontal: 'left' };
    worksheet.getCell('A27').font = {
        bold: true,
        underline: true,
    }

    worksheet.mergeCells('A28:E28');
    worksheet.getCell('A28').value = "( ) ไม่มีนัดพบติดตามผล ( ) มีนัดพบติดตามผลดังตาราง";
    worksheet.getCell('A28').alignment = { horizontal: 'left' };

    worksheet.mergeCells('A29:B29');
    worksheet.getCell('A29').value = "ว/ด/ป";
    worksheet.getCell('A29').alignment = { horizontal: 'center' };
    worksheet.getCell('A29').font = {
        bold: true,
    }
    worksheet.getCell('A29').fill = {
        type: 'pattern',
        pattern: 'darkVertical',
        fgColor: { argb: 'FAFAD2' }
    }

    worksheet.mergeCells('C29:G29');
    worksheet.getCell('C29').value = "ผลที่เกิด/คำแนะนำเพิ่มเติม";
    worksheet.getCell('C29').alignment = { horizontal: 'center' };
    worksheet.getCell('C29').font = {
        bold: true,
    }
    worksheet.getCell('C29').fill = {
        type: 'pattern',
        pattern: 'darkVertical',
        fgColor: { argb: 'FAFAD2' }
    }

    worksheet.mergeCells('H29:I29');
    worksheet.getCell('H29').value = "หมายเหตุ";
    worksheet.getCell('H29').alignment = { horizontal: 'center' };
    worksheet.getCell('H29').font = {
        bold: true,
    }
    worksheet.getCell('H29').fill = {
        type: 'pattern',
        pattern: 'darkVertical',
        fgColor: { argb: 'FAFAD2' }
    }

    worksheet.mergeCells('A30:B30');
    worksheet.mergeCells('A31:B31');

    worksheet.mergeCells('C30:G30');
    worksheet.mergeCells('C31:G31');

    worksheet.mergeCells('H30:I30');
    worksheet.mergeCells('H31:I31');

    worksheet.mergeCells('A32:I32');
    worksheet.getCell('A32').value = "*** เก็บไว้ที่ธุรการภาควิชาฯ/อาจารย์ที่ปรึกษา/ฝ่ายวิชาการและวิจัย คณะวิศวกรรมศาสตร์ ***";
    worksheet.getCell('A32').alignment = { horizontal: 'center' };

    // Draw Table
    worksheet.getCell('A29').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('C29').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('H29').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('A30').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('A31').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('C30').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('C31').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('H30').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    worksheet.getCell('H31').border = {
        top: { style: 'thin' },
        left: { style: 'thin' },
        bottom: { style: 'thin' },
        right: { style: 'thin' }
    };
    // Save
    const buf = await workbook.xlsx.writeBuffer();
    saveAs(new Blob([buf]), 'APPOINTMENT_เอกสารการนัดหมาย' + nameStudent + '.xlsx');
}